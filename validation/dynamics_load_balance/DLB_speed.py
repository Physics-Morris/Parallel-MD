...
ax.plot(extent, speedup1, '-.', markersize=8, color=colors(0), label='uniform_slab')
ax.plot(extent, speedup2, '-*', markersize=8, color=colors(1), label='uniform_slab $+$ rotate')
ax.plot(extent, speedup3, '-v', markersize=8, color=colors(2), label='gaussian_x')
ax.plot(extent, speedup4, '-s', markersize=8, color=colors(3), label='gaussian_x $+$ rotate')
ax.plot(extent, speedup5, '-p', markersize=8, color=colors(5), label='gaussian_xy')
ax.plot(extent, speedup6, '-h', markersize=8, color=colors(6), label='gaussian_xy $+$ rotate')
ax.plot(extent, speedup7, '-X', markersize=8, color=colors(7), label='uniform_sphere')

ax.legend(loc='center left', bbox_to_anchor=(1, 0.5))
ax.set_xlabel('dynamics load balance extent $[0,1]$')
ax.set_ylabel(r'maximum speedup ($\times$)')

plt.tight_layout()
# plt.savefig('extent_speedup_dark.png', dpi=1000, transparent=True)
# plt.savefig('extent_speedup_light.png', dpi=1000)
plt.show()