module diagnostics
    ...
    subroutine output(num, ierr)
        ...
        ! call h5screate_simple_f(rank_mass,   dim_mass,   filespace_id, error)
        ! call h5screate_simple_f(rank_charge, dim_charge, filespace_id, error)
        ! call h5screate_simple_f(rank_vel,    dim_vel,    filespace_id, error)
        ! call h5screate_simple_f(rank_pos,    dim_pos,    filespace_id, error)
        ! call h5screate_simple_f(rank_index,  dim_index,  filespace_id, error)

        !> create the dataset with default properties.
        call h5dcreate_f(file_id, dsetname_id,     h5t_ieee_f64le, filespace_id, dsetid_id, error)
        ! call h5dcreate_f(file_id, dsetname_mass,   h5t_ieee_f64le, filespace_id, dsetid_id, error)
        ! call h5dcreate_f(file_id, dsetname_charge, h5t_ieee_f64le, filespace_id, dsetid_id, error)
        ! call h5dcreate_f(file_id, dsetname_vel,    h5t_ieee_f64le, filespace_id, dsetid_id, error)
        ! call h5dcreate_f(file_id, dsetname_pos,    h5t_ieee_f64le, filespace_id, dsetid_id, error)
        ! call h5dcreate_f(file_id, dsetname_index,  h5t_ieee_f64le, filespace_id, dsetid_id, error)
        ...
        count_id(1)       = dim_id(1)
        count_id(2)       = locals_part_num(coords(1)+1, coords(2)+1, coords(3)+1)
        offset_id(1)      = 0
        offset_id(2)      = calculate_offset(counts_part, my_id)
        ! count_mass(1)     = dim_mass(1)
        ! count_mass(2)     = locals_part_num(coords(1)+1, coords(2)+1, coords(3)+1)
        ! offset_mass(1)    = 0
        ! offset_mass(2)    = calculate_offset(counts_part, my_id)
        ! count_charge(1)   = dim_charge(1)
        ! count_charge(2)   = locals_part_num(coords(1)+1, coords(2)+1, coords(3)+1)
        ! offset_charge(1)  = 0
        ! offset_charge(2)  = calculate_offset(counts_part, my_id)
        ! count_vel(1)      = dim_vel(1)
        ! count_vel(2)      = locals_part_num(coords(1)+1, coords(2)+1, coords(3)+1)
        ! offset_vel(1)     = 0
        ! offset_vel(2)     = calculate_offset(counts_part, my_id)
        ! count_pos(1)      = dim_pos(1)
        ! count_pos(2)      = locals_part_num(coords(1)+1, coords(2)+1, coords(3)+1)
        ! offset_pos(1)     = 0
        ! offset_pos(2)     = calculate_offset(counts_part, my_id)
        ! count_index(1)    = dim_index(1)
        ! count_index(2)    = locals_part_num(coords(1)+1, coords(2)+1, coords(3)+1)
        ! offset_index(1)   = 0
        ! offset_index(2)   = calculate_offset(counts_part, my_id)
    ...
    function calculate_offset(counts_part, id) result(offset)
        implicit none
        integer, dimension(numprocs)  :: counts_part
        integer  :: id, offset, i
